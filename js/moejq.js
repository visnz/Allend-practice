...
// 指定资源进行全部生产,可传入名称与编号,配合 合成资源定位表
...
function GenerateResourceAll(target, count = 4) {
    ...
    i = (typeof target === 'string' ? 合成资源定位表[target] : target)
...
function jobIncrease(job,count=1,increase=true){
    ...
    i = (typeof job === 'string' ? 职业[job] : job)
...
自动打猎 = () => { $("#fastHuntContainerCount").click() }
抬头看天空 = () => { $("#observeBtn").click() }
赞美太阳 = () => { $("#fastPraiseContainer > a:nth-child(1)")[0].click() }
_进入篝火面板 = () => { $("a.tab:nth-child(1)")[0].click() }
_进入管理面板 = () => { $("a.tab:nth-child(3)")[0].click() }
_管理职业 = () => { $("div.panelContainer:nth-child(3) > div:nth-child(3) > table:nth-child(1) > tr:nth-child(1) > td:nth-child(2) > div:nth-child(3) > div:nth-child(1) > span:nth-child(1)").click() }
_提拔小猫 = () => { $("div.panelContainer:nth-child(3) > div:nth-child(3) > table:nth-child(1) > tr:nth-child(1) > td:nth-child(2) > div:nth-child(4) > div:nth-child(1)").click() }
var preIndex = 0
_记录当前面板Index = () => { preIndex = $(".activeTab").parent().children(".activeTab").index() }
_返回记录面板Index = () => { $("a.tab:nth-child(" + (preIndex + 1) + ")")[0].click() }
_检查粮食增量是否为正=()=>{
...
合成木材 = () => { GenerateResourceAll("木材") }
合成钢 = () => { GenerateResourceAll("钢") }
合成木梁 = () => { GenerateResourceAll("木梁") }
合成石板 = () => { GenerateResourceAll("石板") }
合成金属板 = () => { GenerateResourceAll("金属板") }
合成齿轮 = () => { GenerateResourceAll("齿轮") }
合成脚手架 = () => { GenerateResourceAll("脚手架") }
合成船 = () => { GenerateResourceAll("船") }
合成羊皮纸 = () => { GenerateResourceAll("羊皮纸") }
合成手稿 = () => { GenerateResourceAll("手稿") }
合成概要 = () => { GenerateResourceAll("概要") }
合成蓝图 = () => { GenerateResourceAll("蓝图") }
合成巨石 = () => { GenerateResourceAll("巨石") }
...
// 全自动关的功能
$("#全自动关").click((e) => {
    所有操作.map((j) => j.list.map((k) => clearInterval(k.AutoID)))
